...
def get_token_embedders(model_name, tune_bert=False, special_tokens_fix=0):
    ...
    text_filed_emd = BasicTextFieldEmbedder(
        token_embedders=token_embedders, embedder_to_indexer_map=embedder_to_indexer_map, allow_unmatched_keys=True
    )
...
def get_model(
    model_name, vocab, tune_bert=False, predictor_dropout=0, label_smoothing=0.0, confidence=0, special_tokens_fix=0
):
...
def main(args):
    ...
    if args.vocab_path:
        ...
        vocab = Vocabulary.from_instances(
            train_data,
            max_vocab_size={"tokens": 30000, "labels": args.target_vocab_size, "d_tags": 2},
    ...
    instances_per_epoch = (
        None if not args.updates_per_epoch else int(args.updates_per_epoch * args.batch_size * args.accumulation_size)
    )
    ...
    trainer = Trainer(
        ...
        cuda_verbose_step=int(args.cuda_verbose_steps) if args.cuda_verbose_steps else None,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train_set", help="Path to the train data", required=True)
    ...
    parser.add_argument(
        "--vocab_path",
        help="Path to the model vocabulary directory." "If not set then build vocab from data",
        default="",
    )
    parser.add_argument("--batch_size", type=int, help="The size of the batch.", default=32)
    parser.add_argument(
        "--max_len", type=int, help="The max sentence length" "(all longer will be truncated)", default=50
    )
    parser.add_argument("--target_vocab_size", type=int, help="The size of target vocabularies.", default=1000)
    parser.add_argument("--n_epoch", type=int, help="The number of epoch for training model.", default=20)
    parser.add_argument(
        "--patience", type=int, help="The number of epoch with any improvements" " on validation set.", default=None
    )
    parser.add_argument(
        "--skip_correct", type=int, help="If set than correct sentences will be skipped " "by data reader.", default=1
    )
    parser.add_argument(
        "--skip_complex",
        type=int,
        help="If set than complex corrections will be skipped " "by data reader.",
        choices=[0, 1, 2, 3, 4, 5],
        default=0,
    )
    parser.add_argument("--tune_bert", type=int, help="If more then 0 then fine tune bert.", default=1)
    parser.add_argument(
        "--tag_strategy",
    ...
    parser.add_argument("--accumulation_size", type=int, help="How many batches do you want accumulate.", default=4)
    parser.add_argument("--lr", type=float, help="Set initial learning rate.", default=1e-5)
    parser.add_argument(
        "--cold_steps_count", type=int, help="Whether to train only classifier layers first.", default=4
    )
    parser.add_argument("--cold_lr", type=float, help="Learning rate during cold_steps.", default=1e-3)
    parser.add_argument("--predictor_dropout", type=float, help="The value of dropout for predictor.", default=0.0)
    parser.add_argument("--lowercase_tokens", type=int, help="Whether to lowercase tokens.", default=0)
    parser.add_argument("--pieces_per_token", type=int, help="The max number for pieces per token.", default=5)
    parser.add_argument(
        "--cuda_verbose_steps",
    ...
    parser.add_argument(
        "--label_smoothing", type=float, help="The value of parameter alpha for label smoothing.", default=0.0
    )
    parser.add_argument("--tn_prob", type=float, help="The probability to take TN from data.", default=0)
    parser.add_argument("--tp_prob", type=float, help="The probability to take TP from data.", default=1)
    parser.add_argument(
        "--updates_per_epoch",
        type=int,
    ...
    parser.add_argument("--pretrain_folder", help="The name of the pretrain folder.")
    parser.add_argument("--pretrain", help="The name of the pretrain weights in pretrain_folder param.", default="")
    parser.add_argument(
        "--transformer_model",
